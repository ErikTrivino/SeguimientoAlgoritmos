...
def main_busqueda():
    ...
    etiquetas_tamano = [f"{tam:,}" for tam in tamanos]
    num_metodos = len(metodos)
    x = np.arange(len(tamanos))  # posiciones base por grupo
    ...
    for i, (nombre, _, _) in enumerate(metodos):
        tiempos = resultados[nombre]
        desplazamiento = x + (i - num_metodos/2 + 0.5) * ancho
        plt.bar(desplazamiento, tiempos, width=ancho, label=nombre)
        for j, tiempo in enumerate(tiempos):
            plt.text(desplazamiento[j], tiempo + max(tiempos)*0.02, f"{tiempo:.2f}", 
                     ha='center', va='bottom', fontsize=9, bbox=dict(facecolor='white', edgecolor='none', pad=1))

    plt.xticks(x, etiquetas_tamano)
    plt.xlabel("Tamaño del arreglo", fontsize=12)
    plt.ylabel("Tiempo (milisegundos)", fontsize=12)
    plt.title("Comparación de algoritmos de búsqueda (Gráfico de barras)", fontsize=14)
    plt.legend()
